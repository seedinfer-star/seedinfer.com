...
# ENV override prebuild:
# SEEDINFER_PREBUILD_IMAGE=ghcr.io/seedinfer/provider:cuda13.3-nvfp4 SEEDINFER_PREBUILD_URL=https://seedinfer.com/provider-image.tar.gz curl -fsSL https://seedinfer.com/install.sh | bash
# SEEDINFER_SKIP_PREBUILD=1 curl -fsSL https://seedinfer.com/install.sh | bash -s -- --authkey XXX  # wymuś build
```
...
| Chcesz wymusić host (stare zachowanie) | `curl -fsSL https://seedinfer.com/install.sh \| bash -s -- --force-host-tailscale` lub `FORCE_HOST_TAILSCALE=1 ...` — przełączy hosta z `tailscale.com` → Headscale (`--reset`, rozłączy domowy tailnet) |

Logi: `docker compose -f provider/docker-compose.yml logs -f` · `docker exec tailscale-seedinfer tailscale status` (kontener) · `tailscale status` (host — nienaruszony, 100.94.x.x) · `docker logs tailscale-seedinfer` · `curl -fsS http://127.0.0.1:47901/metrics | jq`

---

## Plug-and-play flow (NVFP4) — jedna komenda
...
#  - prebuild: docker pull ghcr.io/seedinfer/provider:cuda13.3-nvfp4 (ghcr, 600s timeout) || Pi registry -> curl https://seedinfer.com/provider-image.tar.gz | docker load (Pi /opt/seedinfer/public) || docker compose up -d --build
#     -> jeśli prebuild OK: docker compose up -d (bez --build, szybki)
#     -> else: docker compose up -d --build (~28GB build + HF 30GB)
...
#        du -sh ./models/cache — rośnie w czasie download
...
## Prebuild / Pi — architektura

Orange Pi 4 Pro (ARM) **nie buduje** CUDA image (28GB, CUDA 13.3, Blackwell) — tylko hostuje:

- **Primary: ghcr.io** — `ghcr.io/seedinfer/provider:cuda13.3-nvfp4` budowany na hoście 5090 x86_64:
  ```bash
  # na hoście 5090:
  ./scripts/publish-provider-image.sh --push --rsync-pi --registry
  # buduje: docker build -f provider/Dockerfile.cuda -t ghcr.io/seedinfer/provider:cuda13.3-nvfp4 -t seedinfer/provider:cuda-0.1.0 .
  # zapis: docker save ghcr.io/seedinfer/provider:cuda13.3-nvfp4 | gzip -1 > /tmp/provider-image.tar.gz
  # push: docker push ghcr.io/seedinfer/provider:cuda13.3-nvfp4
  # rsync: rsync -avz /tmp/provider-image.tar.gz orangepi@100.107.9.52:/opt/seedinfer/public/provider-image.tar.gz
  # cron weekly: 0 3 * * 0 cd /mnt/d/SeedInfer.com && ./scripts/publish-provider-image.sh --push --rsync-pi >> /var/log/seedinfer-publish.log 2>&1
  ```
- **Fallback Pi tar: `https://seedinfer.com/provider-image.tar.gz`** — serwowany przez Next `/api/provider-image` (stream z `/opt/seedinfer/public/provider-image.tar.gz` na Pi, z Range support) → `curl | docker load` w `install.sh`. Caddy proxuje `seedinfer.com` → `app:3000`, więc duży tar idzie przez Next (alternatywnie Caddy file_server z `/opt/seedinfer/public` bezpośrednio dla lepszej wydajności).
- **Fallback Pi registry: `registry:2` na Pi `:5000`** — `infra/registry/docker-compose.registry.yml` (`registry:2.8.3` na `/mnt/nvme/registry`, port `127.0.0.1:5000`, dostęp via Headscale `gateway.seedinfer.ts.net:5000`). `install.sh` próbuje `docker pull $SEEDINFER_REGISTRY/seedinfer/provider:cuda13.3-nvfp4` jeśli `SEEDINFER_REGISTRY` ustawione.
- **Fallback lokalny: `docker compose build`** (~28GB context + 30GB HF) — wolny, ale zawsze działa.

Kolejność w `install.sh`: `ghcr.io pull` (600s timeout) → Pi registry (jeśli `SEEDINFER_REGISTRY`) → Pi tar `curl | docker load` (1800s) → local build. ENV override: `SEEDINFER_PREBUILD_IMAGE`, `SEEDINFER_PREBUILD_URL`, `SEEDINFER_SKIP_PREBUILD=1`, `SEEDINFER_REGISTRY`.
...
docker compose -f infra/registry/docker-compose.registry.yml --profile ui up -d  # opcjonalne UI na :6000
ls -lh /opt/seedinfer/public/provider-image.tar.gz
curl -I https://seedinfer.com/provider-image.tar.gz  # via Next
```
...
provider/
  ...
  docker-compose.yml     # runtime nvidia, volumes modeli, env NVFP4 (VLLM_MODEL=nvidia/...), ports 47900:8000+47901:3001, image seedinfer/provider:cuda-0.1.0 + ghcr tag + sidecar tailscale (profile tailscale, tailscale-seedinfer, volume tailscale-seedinfer-state, network seedinfer-tailnet, healthcheck, cap_add NET_ADMIN/NET_RAW, device /dev/net/tun, TS_AUTHKEY/TS_HOSTNAME/TS_LOGIN_SERVER/TS_EXTRA_ARGS)
...
infra/registry/
  docker-compose.registry.yml # Pi registry:2 na :5000, volume /mnt/nvme/registry
lib/
...
app/api/provider-image/route.ts # GET/HEAD /provider-image.tar.gz — stream z Pi /opt/seedinfer/public (Range support)
...
`publish-provider-image.sh` buduje na 5090 i hostuje na Pi — Pi nie buduje CUDA (ARM). Walidacja: `bash -n install.sh`, `bash -n publish-provider-image.sh`, `docker compose config`, `tsc --noEmit`, `npm run build`.
