...
3. Klonuje `provider/` do `/opt/seedinfer-provider`, tworzy `.env`, próbuje `docker pull ghcr.io/seedinfer/provider:cuda13.3-nvfp4` / Pi tar / `docker compose up -d` (bez --build gdy prebuild OK)
...
provider/
  ...
  docker-compose.yml     # runtime nvidia, volumes modeli, env NVFP4 (VLLM_MODEL=nvidia/...), ports 47900:8000+47901:3001, image seedinfer/provider:cuda-0.1.0 + ghcr tag
