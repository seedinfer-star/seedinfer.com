...
REPO_URL="https://github.com/seedinfer-star/seedinfer.com.git" # public clone (org seedinfer/seedinfer.com 404 - org missing, token login seedinfer-star, use star repo; fallback provider.tar.gz via gateway)
...
if [[ -n "$COMPOSE_FILE" ]]; then
  ...
  grep -v -E "^(TAILSCALE_AUTHKEY|MODEL|VLLM_MODEL|SEEDINFER_GATEWAY_URL|TAILSCALE_LOGIN_SERVER|TAILSCALE_HOSTNAME|HF_TOKEN)=" "$ENV_FILE" > "$ENV_FILE.tmp" 2>/dev/null || cp "$ENV_FILE" "$ENV_FILE.tmp"
